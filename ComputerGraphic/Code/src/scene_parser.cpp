...
SceneParser::SceneParser(const char *filename) {
	...
	//the extending name of the file
    if (strcmp(ext, ".txt") != 0) {
...
AreaLight* SceneParser::parseAreaLight()
{
	...
	getToken(token);
	assert(!strcmp(token, "}"));
	return new AreaLight(center, normal, color, radius);
}
// ====================================================================
// ====================================================================
...
Material *SceneParser::parseMaterial() {
	...
	float shininess = 0, w_reflect = 0, w_transmit = 0, Refraction = 1;
    getToken(token);
    assert (!strcmp(token, "{"));
    while (true) {
        getToken(token);
		...
		else if (strcmp(token, "w_reflect")==0) {
			w_reflect = readFloat();
		}
		else if (strcmp(token, "w_transmit") == 0) {
			w_transmit = readFloat();
		}
	...
	auto* answer = new Material(diffuseColor, specularColor, shininess, w_reflect, w_transmit, Refraction);
    return answer;
}
...
Group *SceneParser::parseGroup() {
    ...
    while (num_objects > count) {
        ...
        if (!strcmp(token, "MaterialIndex")) {
            ...
            answer->addObject(count, object);//count:index of onject in the vector of group
...
Sphere *SceneParser::parseSphere() {
	...
	{
		char textfile[MAX_PARSER_TOKEN_LENGTH];
		getToken(textfile);
		getToken(token);
		assert(!strcmp(token, "}"));
		return new Sphere(center, radius, current_material, textfile);
	}
	else
	{
		assert(!strcmp(token, "}"));
		assert(current_material != nullptr);
		return new Sphere(center, radius, current_material, nullptr);
	}
}
...
Plane *SceneParser::parsePlane() {
	...
	if (!strcmp(token, "Texture"))
	{
		...
		getToken(token);
		assert(!strcmp(token, "}"));
		return new Plane(normal, offset, current_material,textfile);
	}
	else
	{
		assert(!strcmp(token, "}"));
		assert(current_material != nullptr);
		return new Plane(normal, offset, current_material, nullptr);
	}
...
Triangle *SceneParser::parseTriangle() {
	...
	if (!strcmp(token, "Texture"))
	{
		...
		getToken(token);
		assert(!strcmp(token, "}"));
		return new Triangle(v0, v1, v2, normal, normal, normal, current_material,textfile);
	}
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
	return new Triangle(v0, v1, v2, normal, normal, normal, current_material, nullptr);
}
...
Square *SceneParser::parseSquare()
{
	...
	while (true) {
		...
		else if (!strcmp(token, "Texture"))
		{
			...
			getToken(token);
			assert(!(strcmp(token, "}")));
			Square* answer = new Square(controls[1], controls[2], controls[3], controls[4], controls[0], current_material, textfile);
			return answer;
	...
	assert(controls.size() == 5);//first is normal
	Square* answer = new Square(controls[1], controls[2], controls[3], controls[4], controls[0], current_material, nullptr);
	return answer;
}
...
Mesh *SceneParser::parseTriangleMesh() {
	...
	Mesh* answer = new Mesh(filename, current_material, nullptr);
...
Curve* SceneParser::parseBezierCurve()
{
	...
	while (true) {
		...
		else if (!strcmp(token, "Texture"))
		{
			...
			getToken(token);
			assert(!(strcmp(token, "}")));
			Curve* answer = new BezierCurve(controls, current_material, textfile);
			return answer;
	...
	Curve* answer = new BezierCurve(controls, current_material, nullptr);
	return answer;
}
